 ...
 *
 * $Id: ImageConv.php 3809 2008-12-18 12:48:41Z OscarB $
 *
 * @package    Image
 * @author     Kohana Team
...
class ImageConv {
	...
	/**
	 * Creates a new ImageConv instance and returns it.
	 *
	...
	{
		return new ImageConv($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(ImageConv::$allowed_types[$image_info[2]]))
			throw new CException('image type not allowed');

		// ImageConv has been validated, load it
		$this->image = array
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => ImageConv::$allowed_types[$image_info[2]],
	 ...
	 * wish to use height as master dim, set $image->master_dim = ImageConv::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: ImageConv::NONE, ImageConv::AUTO, ImageConv::WIDTH, ImageConv::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== ImageConv::NONE AND
				    $value !== ImageConv::AUTO AND
				    $value !== ImageConv::WIDTH AND
				    $value !== ImageConv::HEIGHT)
					return FALSE;
			break;
		}
